    ...
    this.balls.forEach((ball, ballIdx) => {
      this.balls.forEach((otherBall, otherBallIdx) => {
        ...
        if (r_2 <= (ball.r + otherBall.r) ** 2) {
          tmpVelocity[ballIdx] = {
            x: (tmpVelocity[ballIdx] && tmpVelocity[ballIdx].x) + ((ball.mass - otherBall.mass) * ball.velocity.x + 2 * otherBall.mass * otherBall.velocity.x) / (ball.mass + otherBall.mass),
            y: (tmpVelocity[ballIdx] && tmpVelocity[ballIdx].y) + ((ball.mass - otherBall.mass) * ball.velocity.y + 2 * otherBall.mass * otherBall.velocity.y) / (ball.mass + otherBall.mass)
          }
        } else {
          let r_2_5 = r_2 ** 2.5
